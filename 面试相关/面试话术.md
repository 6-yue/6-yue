...
熟练使用 HTML + CSS JavaScript独立完成静态页面布局

熟练掌握 HTML5、CSS3 新特性

能够灵活运用ES6

熟练使用Vue框架，Vue+Vuex+Vue-router

熟练使用ElementUI组件库进行页面布局

熟练使用移动端弹性布局，百分比布局

熟练掌握webpack前端模块化打包工具

熟练使用Ajax,axios前后端交互技术

最近的一个项目是，后台管理系统，可以系统化显示员工信息，到岗情况，入职及离职管理
在这个项目里我负责的模块有：用户登录，首页模块，查询模块，员工管理模块
主要用到的技术栈有：vue全家桶，elementUI,echarts,axios
```
...
4. 熟练使用Vue框架，Vue+Vuex+Vue-router
   ...
   2. vue最突出的优势可以对数据进行双向数据绑定
      1. 双向绑定的原理
         vue 是采用数据支劫持结合发布者-订阅者模式的方式，通过Object.defineProperty()来劫持各个属性的setter,getter,在数据变动时发布消息给订阅者，触发相应的监听回调
         1. 需要observe的数据对象进行递归遍历，包括子属性对象的属性，都加上setter和getter,这样的话，给这个对象的某个值赋值，就会触发setter,那么就能监听到数据变化
         2. compile解析模板指令，将模板中的变量替换成数据，然后初始化渲染页面视图，并将每个指令对应的节点绑定更新函数，添加监听数据的订阅者，一旦数据有变动，收到通知，更新视图
         3. Watcher订阅者是Observer和compile之间通信的桥梁，主要做的事情是：
            1. 在自身实例化时往属性订阅器(dep)里面添加自己
            2. 自身必须有一个update()方法
            3. 待属性变动 dep.notice()通知时,能调用自身的uppdate(方法,并触发 Compile 中绑定的回调,则功成身退。
         4. MVVM 作为数据绑定的入口,整合 Observer、Compile 和Watcher 三者,通 Observer 来监听自己的 model 数据变化,通过 Compile 来解析编译模板指令,最终利用 Watcher 搭起Observer和 Compile 之间的通信桥梁,达到数据变化 -> 图更新;视图交互变化(input) -> 数据 model变更的双向绑定效果
   3. 使用vue编写出来的界面效果本身便是响应式的，这使网页在不同设备上都能显出非常好看的效果
      1. 响应式的原理，什么叫响应式数据
         1. 响应式数据，数据发生了变化，页面会随之进行渲染，此时这样的数据叫做响应式数据
   4. 相比传统的页面通过超链接实现页面的切换和跳转，Vue使用路由不会刷新页面。
...
8. 熟练使用Ajax,axios前后端交互技术
   ...
   1. Ajax不是特指一种技术，而是多种技术结合起的统称，能够在不刷新页面的情况下实现数据的局部更新
   2. axios，是Ajax的一种实现方式
...
组员
   ...
   项目描述: 针对微信端小程序需要的试题数据、视频讲解进行管理，所以开发一个后台        
   管理系统该后台管理系统包含：后台管理，用户管理，角色管理，题库管理，学科管理          
   等功能。

   开发周期 ：3.5月

   技术栈 ：Vue全家桶+ECharts+Element-ui+JWT

  项目职责 : 学科列表模块、目录模块、标签模块、登录等模块


   1、学科列表模块 

         1.1 实现清除功能、删除学科功能、筛选功能、添加学科功能、修改学科功能 

         1.2 实现学科分类与学科标签功能，点击时按照所携带数据进行跳转，显示

         1.3 实现分页功能，以及输入页码查询功能
    ...
    2、目录、标签列表模块
         ...
         2.2 实现筛选功能根据目录名称和状态进行搜索（状态为启用和禁用）
    ...
    3、负责新功能的开发、维护优化代码、分析解决 bug、提升用户体验

    4、与后端对接实现用户账号密码登录、微信授权登录
```
